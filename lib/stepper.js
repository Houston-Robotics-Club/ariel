...
function Stepper(opts) {
  ...
  if (Array.isArray(this.pins)) {
    var pinArray = this.pins;
    if (pinArray.length === 2 || pinArray.length === 3) {
    ...
    if (pinArray.length & 0x01) {
      this.pins.enable = pinArray.pop();
    }

    if (opts.type === Stepper.TYPE.DRIVER) {
      this.pins.step = pinArray[0];
      this.pins.dir = pinArray[1];
    } else {
      this.pins = new MotorPins(pinArray);
    }
  }
  ...
  if (this.pins.enable) {
    this.io.pinMode(this.pins.enable, this.io.MODES.OUTPUT);
  }

  if (this.pins.enableA) {
    this.io.pinMode(this.pins.enableA, this.io.MODES.OUTPUT);
  }
...
Stepper.prototype.step = function(stepsOrOpts, callback) {
  ...
  if (isValidStep) {
    ...
    params.push(function(complete) {
      ...
      ["enable", "enableA", "enableB"].forEach(function (pinName) {
        if (this.pins[pinName]) {
          this.io.digitalWrite(this.pins[pinName], 0);
        }
      }.bind(this));
    ...
    ["enable", "enableA", "enableB"].forEach(function (pinName) {
      if (this.pins[pinName]) {
        this.io.digitalWrite(this.pins[pinName], 1);
      }
    }.bind(this));
